...
export const proxyAgentPoll = async (req, res) => {
  try {
    ...
    if (agentId === 'local' || process.env.USE_LOCAL_AGENTS === 'true') {
      console.log("[Proxy] Using local agent implementation instead of external API");
      return handleLocalAgentRequest(req, res);
    }

    // If taskId is missing for a poll request, return an error
    if (!taskId) {
      console.error("[Proxy] Missing taskId parameter for poll request:", { body: req.body });
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      console.error(`[Proxy] Error making request to ${finalEndpoint}:`, error.message);
...
const handleDirectAgentRequest = async (req, res) => {
  try {
    ...
    console.log(`[Proxy] Sending agent request to ${endpoint} with payload:`, payload);
    ...
    try {
      ...
      const responseData = response.data;
      console.log(`[Proxy] Task creation response from ${agentId}:`, responseData);
  ...
  try {
    ...
    if (taskId && !question) {
      try {
        ...
        const formattedHtml = formatAgentResponse(result, "Local Agent (Fallback)");
    ...
    return res.status(500).json({
      success: false, 
      status: "error",
      error: error.message || "An unknown error occurred in local agent processing"
    });
  }
};