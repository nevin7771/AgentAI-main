...
export const testAgentConfig = (req, res) => {
  try {
    ...
    const jwtIssuer = process.env[`${agentId.toUpperCase()}_JWT_ISSUER`] || 
      process.env.JWT_ISSUER || 
      "yana.bao+AIStudio+DG01@test.zoom.us";
    
    const jwtAudience = process.env[`${agentId.toUpperCase()}_JWT_AUDIENCE`] || 
      process.env.JWT_AUDIENCE || 
      "zoom_caic";
    
    const jwtAid = process.env[`${agentId.toUpperCase()}_JWT_AID`] || 
      process.env.JWT_AID || 
      "3v8eT3vkQ1-PBQnN61MJog";
    
    const jwtUid = process.env[`${agentId.toUpperCase()}_JWT_UID`] || 
      process.env.JWT_UID || 
      "NhiGO2feQEORV5Loghzx_Q";
    
    const secretKey = process.env[`${agentId.toUpperCase()}_JWT_SECRET`] || 
      process.env.JWT_SECRET_KEY || 
      "gzazjvdts768lelcbcyy5ecpkiguthmq";
    
    // Get API endpoint
    const apiBaseUrl = process.env.API_BASE_URL || 
      "https://dg01ai.zoom.us/open/api/v1/caic/general-ai/MRlQT_PPQM2nZVaQVflhFw";
    
    const endpoint = process.env[`${agentId.toUpperCase()}_API_URL`] || 
    ...
    res.status(200).json({
      ...
      config: {
        ...
        jwt: {
          ...
          aid: jwtAid,
          uid: jwtUid,
          secretKey: secretKey.substring(0, 3) + '***' + secretKey.substring(secretKey.length - 3),
        },
    ...
    res.status(500).json({
      success: false,
      error: error.message || "An error occurred while testing agent configuration",
    });
  }
};