// public/src/store/agent-actions.js - ENHANCED JIRA AGENT WITH CONVERSATION CONTINUITY - FIXED UNUSED VARS
...
export const sendAgentQuestion = (questionData) => {
  return async (dispatch, getState) => {
    ...
    // Route Jira Agent to dedicated handler
    ...
    try {
      ...
      console.log(
        `[sendAgentQuestion] Processing with agent: ${selectedAgent}`
      );

      // For now, return success for other agents
      dispatch(uiAction.setLoading(false));
      dispatch(agentAction.setLoading(false));
...
// ENHANCED: Dedicated Jira Agent handler with conversation continuity - FIXED UNUSED VARS
export const sendJiraAgentQuestion = (questionData) => {
  return async (dispatch, getState) => {
    ...
    // FIXED: Removed unused previousChat variable
    ...
    dispatch(
      chatAction.chatStart({
        useInput: {
          ...
          isSearch: true,
          searchType: "agent",
          agentType: "jira_ag", // Store agent type for sidebar tagging
          queryKeywords: queryKeywords,
          sources: [],
          relatedQuestions: [],
          isPreformattedHTML: false,
    ...
    // Only navigate if no existing conversation
    ...
    try {
        ...
        dispatch(
          chatAction.chatStart({
            useInput: {
              ...
              isSearch: true,
              searchType: "agent",
              agentType: "jira_ag", // Keep agent type
              queryKeywords: queryKeywords,
              sources: [],
      ...
      // Enhanced chat history management for conversation continuity
      ...
      if (currentChatHistoryId) {
        ...
        try {
          ...
          const appendResponse = await fetch(appendUrl, {
            ...
            body: JSON.stringify({
              ...
              isSearch: true,
              searchType: "agent",
              agentType: "jira_ag", // Store agent type in backend
            }),
          });
        ...
        try {
          ...
          const historyResponse = await fetch(createHistoryUrl, {
            ...
            credentials: "include",
            body: JSON.stringify({
              title: `Jira: ${question.substring(0, 40)}`, // Clear Jira title
              message: {
                user: question,
                gemini: responseContent,
                sources: sources,
      ...
      if (finalChatHistoryId) {
        ...
        // Navigate to proper URL only for new conversations
        if (
      ...
      dispatch(
        chatAction.chatStart({
          useInput: {
            user: question,
            gemini: `
              <div class="search-results-container error">
            ...
            isSearch: true,
            searchType: "agent",
            agentType: "jira_ag", // Keep agent type even in errors
            queryKeywords: queryKeywords,
            sources: [],
            relatedQuestions: [],
            error: true,
